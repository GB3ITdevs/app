...
public class AdminIDJSONParser {
	public static List<AdminID> parseFeed(String content) {
		...
		try {
			...
			for (int i = 0; i < ar.length(); i++) {
				...
				// Set each JSON adminID field to the appropriate adminID object
				// field
				adminID.setAdminID(obj.getInt("adminID"));
				adminID.setUserID(obj.getInt("userID"));
	...
	// Method to parse the Android adminID object to JSON adminID format in
	// order to post
	public static String POST(AdminID adminID) {
		...
		try {
			// For each field in the parsed in adminID object, set it to the
			// appropriate JSON object field
			jsonAdminID.accumulate("userID",
					Integer.toString(adminID.getUserID()));
