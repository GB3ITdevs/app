	...
	public static List<ContactInfo> parseFeed(String content) {
		...
		try {
			...
			for (int i = 0; i < ar.length(); i++) {
				...
				// Set each JSON ContactInfo field to the appropriate
				// ContactInfo object field
=======
public class ContactInfoJSONParser 
{
...
<<<<<<< HEAD
	...
	// Method to parse the Android ContactInfo object to JSON ContactInfo format
	// in order to post
	public static String POST(ContactInfo contactInfo) {
		...
		try {
			// For each field in the parsed in ContactInfo object, set it to the
			// appropriate JSON object field
			jsonContactInfo.accumulate("postalCode",
					Integer.toString(contactInfo.getPostalCode()));
			jsonContactInfo.accumulate("suburb", contactInfo.getSuburb());
			jsonContactInfo.accumulate("city", contactInfo.getCity());
			jsonContactInfo.accumulate("streetNumber",
					Integer.toString(contactInfo.getStreetNumber()));
